  ...
  return (
    <div className="space-y-[5.6rem] lg:space-y-[8rem]">
      {items.map((item, index) => {
        switch (item.type) {
          case 'single':
            return <SingleRow key={index} item={item} />
          case 'double':
            return <DoubleRow key={index} item={item} />
          case 'doubleWithText':
            return <DoubleWithTextRow key={index} item={item} />
          case 'carousel':
            return <CarouselRow key={index} item={item} />
          default:
            return null
        }
      })}
...
function SingleRow({ item }: { item: Extract<GalleryItem, { type: 'single' }> }) {
  return (
    <figure>
      {item.parallax ? (
        <ImageParallax src={item.image} alt={item.alt ?? ''} aspectClass="aspect-video" />
      ) : (
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-[2rem] lg:gap-[3.5rem]">
      {item.images.map((img, i) => (
        <div key={i} className="relative aspect-[3/4] overflow-hidden bg-stone-200">
          <Image
            src={img.src}
            alt={img.alt ?? ''}
            fill
  ...
  return (
    ...
    >
      ...
      <div className={imageFirst ? 'lg:pl-[3rem]' : 'lg:pr-[3rem]'}>
        <blockquote
          className="font-serif text-[clamp(1.5rem,2.5vw,2.4rem)] leading-[1.5] lg:indent-[12rem]"
        >
          {item.text}
        </blockquote>
      </div>
    ...
    <div className="relative">
      ...
      <div className="flex gap-[2rem] overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-4">
        {item.images.map((img, i) => (
          <div
            key={i}
            className="flex-none w-[80vw] md:w-[50vw] lg:w-[33vw] snap-start relative aspect-[3/4] overflow-hidden bg-stone-200"
          >
            <Image
              src={img.src}
              alt={img.alt ?? ''}
